...
class RequerimentoServico implements AnexoInterface
{
    public function getTitulo(): string
    {
        return 'Anexo 2C - Requerimento Diversos';
    }
    public function getTemplatePath(): string
    {
        return storage_path('app/templates/Anexo2C-N211.pdf');
    }
    ...
    public function getFormSchema(): array
    {
        return [
            CheckboxList::make('solicitacao')->label('Natureza do Requerimento')->columns(2)->required()->live()
                ->options([
                    'inscricao' => 'Inscrição de Embarcação',
                    'cancelamento' => 'Cancelamento de Inscrição',
                    'licencaconstrucao' => 'Licença de Construção',
                    'licencaalteracao' => 'Licença de Alteração',
                    'licencareclassificacao' => 'Licença de Reclassificação',
                    'transfpropriedade' => 'Transferência de Propriedade',
                    'transfjurisdicao' => 'Transferência de Jurisdição',
                    'transfpropjurisdicao' => 'Transferência de Propriedade e Jurisdição',
                    'mudancanome' => 'Mudança de Nome da Embarcação',
                    'renovacaotie' => 'Renovação de TIE/TIEM',
                    'segviatie' => '2ª Via de TIE/TIEM',
                    'alteracaodadosembarcacao' => 'Alteração de dados cadastrais da embarcação',
                    'alteracaodadosproprietario' => 'Alteração de dados cadastrais do Proprietário',
                    'trocamotor' => 'Troca do Motor Propulsor',
                    'certidaoembarcacao' => 'Certidão Relativa à Situação da Embarcação',
                    'registroaverbacao' => 'Registro de Ônus e Averbações',
                    'cancaverbacao' => 'Cancelamento do Registro de Ônus',
                    'vistoriaarqueacao' => 'Vistoria de Arqueação',
                    'vistoriarearqueacao' => 'Vistoria de Rearqueação',
                    'vistoriaclassificacao' => 'Vistoria para alterar classificação',
                    'outrosservicos' => 'Outros Serviços',
                ]),
            Grid::make(3)->schema([
                TextInput::make('texto1')->label('1ª Opção de Nome'),
                TextInput::make('texto2')->label('2ª Opção de Nome'),
                TextInput::make('texto3')->label('3ª Opção de Nome'),
    ...
    public function getDados($record, array $input): array
    {
        ...
        $dados = [
            ...
            'logradouro' => $this->up($cliente->logradouro),
            'numero' => $cliente->numero ?? '',
            'aptosala' => $this->up($cliente->complemento),
            'cidade' => $this->up($cliente->cidade),
            'uf' => $this->up($cliente->uf),
            'cep' => $cliente->cep ?? '',
            'rg' => $this->up($cliente->rg),
            'orgexpedidor' => $this->up($cliente->org_emissor),
            'cpfcnpj' => $cliente->cpfcnpj ?? '',
            'telefone' => $this->up($cliente->telefone ?? $cliente->celular),
            'cpfrequerente' => $cliente->cpfcnpj ?? '',
            'nomeembarcacao' => $this->up($embarcacao->nome_embarcacao),
            'numinscricao' => $this->up($embarcacao->num_inscricao),
            'comprimento' => $embarcacao->comp_total ? number_format($embarcacao->comp_total, 2, '.', '') . 'm' : '',
            'numcasco' => $this->up($embarcacao->num_casco),
            'classificacao' => $this->up($embarcacao->tipo_atividade),
        ...
        if (in_array('renovacaotie', $selecionados)) {
            ...
            if ($sub === 'sim')
                $dados['check_renovacaotie_sim'] = 'Sim';
            if ($sub === 'nao')
                $dados['check_renovacaotie_nao'] = 'Sim';
        }
        if (in_array('segviatie', $selecionados)) {
            $motivo = $input['motivo_segvia'] ?? null;
            if ($motivo) {
                $dados['check_segviatie_' . $motivo] = 'Sim';
            }
        }
